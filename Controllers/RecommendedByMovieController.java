...
public class RecommendedByMovieController
{
    ...
    public void recomendarPorPelicula() {
        ...
        try {
            final Result result = db.execute("MATCH (m:Movie)WHERE m.id = '" + this.movieId + "'" + "MATCH (m2:Movie)" + "WHERE (m2.genre = m.genre or m2.actor = m.actor or m2.director = m.director or m2.producer = m.producer) and m2.id <> m.id and m2.popularity >= m.popularity " + "RETURN m2.id, m2.name LIMIT 10");
            tx.success();
            if (result.hasNext()) {
                ...
                while (result.hasNext()) {
                    ...
                    button.setOnAction((EventHandler)new EventHandler<ActionEvent>() {
                        public void handle(final ActionEvent event) {
                            ...
                            RecommendedByMovieController.access$0(RecommendedByMovieController.this, new Main());
                            RecommendedByMovieController.this.main.changeToSelectedMovie(RecommendedByMovieController.this.userLoggedIn, movieId);
                        }
                    });
    ...
    public void llenarClase() {
        ...
        this.recomendarPorPelicula();
    }
    
    static /* synthetic */ void access$0(final RecommendedByMovieController recommendedByMovieController, final Main main) {
        recommendedByMovieController.main = main;
    }
}
